# Avalanche Node Monitoring

Grafana dashboards and installer script for monitoring Avalanche nodes.

## Quick Start

```bash
curl -sSfLO https://raw.githubusercontent.com/Benqi-fi/avalanche-monitoring/master/grafana/monitoring-installer.sh
chmod +x monitoring-installer.sh
./monitoring-installer.sh --help
```

## Full Setup Guide

For complete setup instructions (Prometheus, Grafana, node_exporter), see the [Avalanche monitoring documentation](https://build.avax.network/docs/nodes/maintain/monitoring).
